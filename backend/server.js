...
// German image proxy that resolves a STORED photo URL back to its object key.
// Accepts any shape getImageUrl() has ever produced — the Express proxy path
// (/api/german/images/...) or a direct MinIO/S3 URL ({PUBLIC_ENDPOINT}/{bucket}/key)
// — so browsers never need to reach MinIO directly (fixes broken <img> when
// PUBLIC_STORAGE_ENDPOINT points to a host that is not browser-reachable).
// Usage: GET /api/german/images/url?u=<encodeURIComponent(photoUrl)>
// NOTE: this route MUST be registered before /api/german/images/:key(*) below.
app.get('/api/german/images/url', async (req, res) => {
  const u = req.query.u || '';
  const objectKey = storage.getKeyFromUrl(u);
  if (!objectKey) {
