...
function normalizeColorR(value) {
...
function parseStyleR(el) {
  ...
  const color = normalizeColorR(props['color']);
  if (color && color !== '#000000') out.color = color;
  const bg = normalizeColorR(props['background-color'] || props['background']);
...
function inlineRunsR(root, base = {}) {
  ...
  const walk = (node, fmt) => {
    ...
    if (tag === 'font') {
      const fc = normalizeColorR(node.getAttribute('color'));
      if (fc) nf.color = fc;
    }
    if (tag === 'ul' || tag === 'ol') {
