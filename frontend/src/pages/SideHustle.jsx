    ...
    for (const log of Object.values(logs || {})) {
      ...
      const taskMins = tasksArr
        .filter(t => {
          ...
          return cats.some(c => c === 'Side Hustle');
        })
        .reduce((sum, t) => sum + (parseFloat(t.deepWorkHours) || 0) * 60, 0);
        
      // 2. Timer deepWorkSessions focus blocks
      let timerMins = 0;
      if (log.hustle?.deepWorkSessions) {
        const focusBlocks = log.hustle.deepWorkSessions.filter(b => b.type === 'focus');
        timerMins = focusBlocks.length * 20;
      }
      
      // 3. Manual time spent in the Daily Log
      const manualMins = parseTimeToMinutes(log.hustle?.time);
