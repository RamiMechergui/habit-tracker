...
export default function DailyLog() {
  ...
  // ── Auto-fill Social Media & Phone Usage from Android usage stats ──
  ...
  const fetchUsageStats = useCallback(async () => {
    ...
    try {
      ...
      if (!granted) {
        console.log('[UsageStats] Permission not granted — showing settings prompt');
        return;
      }
      ...
      setLog(prev => {
        ...
        if (socialMin > 0) {
          newBad.social = { ...(newBad.social || {}), min: socialMin };
        }
      ...
      console.warn('[UsageStats] Failed to fetch:', err);
    } finally {
  ...
  const openUsageSettings = async () => {
    try {
      ...
      console.warn('[UsageStats] Could not open settings:', err);
    }
  };
  ...
  const handleDownloadPDF = () => {
    ...
    try {
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(`Generated by Evolvio Habit Tracker • ${new Date().toLocaleDateString()}`, 16, 290);
      ...
      <div className="grid-2">
        <div className="flex-col gap-6">
          ...
          <div className="glass-card p-6 section-bad" style={{ background: 'linear-gradient(145deg, var(--bg-card), rgba(16, 185, 129, 0.03))' }}>
            <div className="flex justify-between items-center mb-5">
              <h3 className="m-0 flex items-center gap-2">🚫 Bad Habits</h3>
              <div className="flex items-center gap-2">
                {isToday && (
                  <button onClick={fetchUsageStats} disabled={usageStatsLoading} title="Refresh phone usage data" style={{
                    background: 'none', border: 'none', cursor: usageStatsLoading ? 'wait' : 'pointer',
                    color: 'var(--text-muted)', padding: 4, borderRadius: '6px', display: 'flex', alignItems: 'center', gap: 4,
                    fontSize: '0.7rem', fontWeight: 600,
                  }}>
                    <RefreshCw size={13} style={{ animation: usageStatsLoading ? 'spin 1s linear infinite' : 'none' }} />
                    {usageStatsLoading ? 'Loading...' : 'Phone Data'}
                  </button>
                )}
            ...
            {isToday && !usagePermissionGranted && (
              <div className="flex items-center gap-3 mb-4 p-3" style={{ background: 'rgba(251, 191, 36, 0.1)', borderRadius: 'var(--radius-sm)', border: '1px solid rgba(251, 191, 36, 0.3)' }}>
                <span style={{ fontSize: '1.2rem' }}>📱</span>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', margin: 0 }}>
                    <strong style={{ color: '#f59e0b' }}>Usage Access needed</strong> to auto-fill Social Media &amp; Phone Usage times.
                  </p>
                  <button onClick={openUsageSettings} style={{
                    marginTop: 6, padding: '4px 12px', borderRadius: '8px', border: 'none',
        ...
        <div className="flex-col gap-6">
          ...
          <div className="glass-card p-6" style={{ background: 'linear-gradient(145deg, var(--bg-card), rgba(99, 102, 241, 0.05))' }}>
            ...
            {videoWarning && (
              <span style={{ color: '#ef4444', fontSize: '0.75rem', marginTop: '8px', display: 'block', animation: 'evolvio-shake 0.4s ease' }}>
                ⚠️ Fill Task & Time to check this box.
              </span>
            )}
                ...
                <div className="expense-row-2">
                  ...
                  >
                    {expenseCategories.map(cat => {
                      ...
                      return <option key={catName} value={catName}>{catIcon} {catName}</option>;
                    })}
                  </select>
