...
import {
  ...
  ShoppingBag, DollarSign, Link as LinkIcon, AlertTriangle, ChevronDown
} from 'lucide-react';
...
const formatPrice = (n, currencyCode = 'USD') => {
  if (n == null || n === '') return '';
...
export default function Wishlist() {
  const { wishlist, fetchWishlist, addWishlistItem, updateWishlistItem, deleteWishlistItem } = useHabits();
  ...
  const currencyTotals = wishlist.reduce((acc, i) => {
            ...
            {wishlist.map(item => {
              ...
              return (
                <div key={item._id} className="glass-card wl-card" style={{
                  ...
                  border: deleteConfirmId === item._id ? '1.5px solid rgba(239,68,68,0.4)' : '1px solid transparent',
                }}>
                  {/* Photo */}
                  <div style={{
                  ...
                  <div style={{ padding: isMobile ? '14px' : '16px', flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    {hasPrice && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontWeight: 700, fontSize: '1.2rem', color: 'var(--accent-emerald)' }}>
