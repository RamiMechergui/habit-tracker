...
pkill -9 -f DynamoDBLocal 2>/dev/null || true
sleep 3

echo "=== Verify DynamoDB is dead ==="
pgrep -f DynamoDBLocal && echo "STILL RUNNING" || echo "DEAD"

echo "=== Delete data ==="
rm -rf ~/dynamodb-data
ls ~/dynamodb-data 2>/dev/null && echo "STILL EXISTS" || echo "DELETED"

echo "=== Create fresh data dir ==="
...
pgrep -f DynamoDBLocal
curl -s -X POST http://localhost:8000 -H "Content-Type: application/x-amz-json-1.0" -H "X-Amz-Target: DynamoDB_20120810.ListTables" -d "{}"
echo ""
...
async function main() {
  ...
  if (existing) {
    console.log('WARN: User already exists, deleting...');
    const { docClient } = require('./db/dynamodb');
    const { DeleteCommand } = require('@aws-sdk/lib-dynamodb');
    await docClient.send(new DeleteCommand({ TableName: 'HabitUsers', Key: { userId: existing.userId } }));
  }
